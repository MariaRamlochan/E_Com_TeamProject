...
class Message extends \app\core\Model{
    ...
    public function getReadPrivate($profile_id){
	...
	public function getUnread($profile_id){
		$SQL = 'SELECT * FROM `message` where read_status="unread" && receiver = :profile_id';
		$STMT = self::$_connection->prepare($SQL);
	...
	public function getMessagesReceived($profile_id){
	...
	public function insert(){
		...
		$STMT = self::$_connection->prepare($SQL);
		$STMT->execute(['sender'=>$this->sender, 'receiver'=>$this->receiver, 'message'=>$this->message, 'read_status'=>$this->read_status, 'private_status'=>$this->private_status]);
	}
